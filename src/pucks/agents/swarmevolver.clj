...
(defn swarmevolver-proposals [p]
  (if-let [g (:genome (:memory p))]
    (let [selves (filter #(self p % (:color-difference g))
                         (filter :mobile (:sensed p)))
          others (filter #(not (self p % (:color-difference g)))
                        (filter :mobile (:sensed p)))
      ...
      (merge {:acceleration 1
              ...
              :rotation (relative-position->rotation 
                          (+v (rotation->relative-position (:rotation p)) 
                              (if (empty? selves)
                                [0 0]
                                (*v (:self-flock-urge g)
                                    (limit1 (apply avgv (map :velocity selves)))))
                              (if (empty? others)
                                [0 0]
                                (*v (:other-flock-urge g)
                                    (limit1 (apply avgv (map :velocity others)))))                           
                              (if (empty? selves)
                                [0 0]
                                (*v (:self-cluster-urge g)
                                    (limit1 (apply avgv (map :position selves)))))                              
                              (if (empty? others)
                                [0 0]
                                (*v (:other-cluster-urge g)
                                    (limit1 (apply avgv (map :position others)))))      
                              (if (empty? vents)
                                [0 0]
                                (*v (:vent-urge g)
                                    (limit1 (apply avgv (map :position vents)))))
                              (if (empty? zappers)
                                [0 0]
                                (*v (:zapper-urge g)
             ...
             (if (> (* (max (:spawn-probability g) 0.01)
               ...
               {})
             {:fire-torpedo (or (and (> (/ (:shoot-self-probability g) 10) 
                                        (rand))
                                     (some #(facing p %) selves))
                                (and (> (/ (:shoot-other-probability g) 10) 
                                        (rand))
                                     (some #(facing p %) others)))}))))
...
(defn mutate
  ...
  (-> genome
    ...
    (assoc :shoot-self-probability (mutate-probability (:shoot-self-probability genome)))
    (assoc :shoot-other-probability (mutate-probability (:shoot-other-probability genome)))
  ...
  (merge (active)
         {:swarmevolver true
          ...
          :memory {:genome {:color (rand-color)
                            ...
                            :shoot-self-probability (rand-probability)
                            :shoot-other-probability (rand-probability)
